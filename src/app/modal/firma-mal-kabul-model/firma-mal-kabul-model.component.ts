  ...
  irsparcala(qrData?: string) {
    ...
    if (parsedData.success) {
      ...
      if (parsedData.soforAdSoyad) {
        const adSoyad = parsedData.soforAdSoyad.split(' ');
        if (adSoyad.length >= 2) {
          this.postorder.depo.sofor!.adi = adSoyad[0];
          this.postorder.depo.sofor!.soyAdi = adSoyad.slice(1).join(' ');
        } else {
          this.postorder.depo.sofor!.adi = parsedData.soforAdSoyad;
        }
      }
      
      if (parsedData.cariKod) {
        this.postorder.muhatapFirma.cariKodu = parsedData.cariKod;
        // Firma otomatik aranabilir
       // this.firmaAra();
      }
      
      if (parsedData.muhatapAdi) {
        this.postorder.muhatapFirma.yetkiliadisoyadi = parsedData.muhatapAdi;
      }
...
console.log(toplamKarakter);
  ...
  qrSonuclariniTemizle() {
    ...
    this.postorder.muhatapFirma.yetkiliadisoyadi = '';
    this.qrParsed.set(false);
    this.qrGorunurVeri.set("");
    this.qrIrsaliyeNo.set("");
  }
