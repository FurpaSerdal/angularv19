...
@Component({
  ...
  styleUrl: './company-inbound-shipments.css',
  providers: [DatePipe]
})
...
constructor(
  ...
  loadData(): void {
    ...
    this.goodreceiptnotesService.getCompanyReceipts(this.altmenuid(),"bugun").subscribe({
      next: (data: any) => {
        this.DataSource.data = data.malKabuller;
  ...
  onDateChanged(): void {
   const baslangic = this.datePipe.transform(this.dateRange.get('start')?.value, 'yyyy-MM-dd');
    ...
    if (baslangic && bitis) { 
      this.yukleniyor.set(true);
      this.goodreceiptnotesService.getCompanyReceipts(this.altmenuid(), zamanlama).subscribe({
        next: (data: any) => {
          this.DataSource.data = data.malKabuller;
          this.yukleniyor.set(false);
        },
