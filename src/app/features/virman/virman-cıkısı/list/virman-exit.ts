...
export class VirmanExit {
  ...
  displayedColumns = ['seri', 'sira', 'tarih'];
...
constructor(
  ...
  loadData(): void {
    ...
    this.virmansTransaction.getvirmans(this.gorevid(), "bugun").subscribe({
      next: (data: any) => {
  ...
  onDateChanged(): void {
    ...
    if (baslangic && bitis) {
      this.yukleniyor.set(true);
      this.virmansTransaction.getvirmans(this.gorevid(), zamanlama).subscribe({
        next: (data: any) => {
          this.DataSource.data = data.evraklar;
          this.yukleniyor.set(false);
        },
  ...
  taskDetay(seri: string, sira: number): void {
    this.yukleniyor.set(true);
    this.virmansTransaction.detailsVirmans(this.gorevid(), seri, sira).subscribe({
      next: (data: any) => {
        this.yukleniyor.set(false);
        this.dialog.open(DetailVirmanExit, {
          width: "50%",
          height: "70%",
          data: data
        });
  ...
  yeniEvrak(): void {
    ...
    this.dialog.open(NewVirmanExit, {
